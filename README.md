...
**Quizzy** is a dynamic Flutter application that uses the **Gemini API** to generate unique quizzes on the fly. It features an engaging user interface enhanced with high-quality **Rive** animations.

## ✨ Features

* **AI-Powered Questions**: Generates endless quiz questions on any topic using Google's Gemini API.
* **Interactive Animations**:
    * `ai_loading.riv`: Engaging loading state while fetching questions.
...
3.  **Setup Environment**
    * Configure your Gemini API key in your project (ensure you do not commit your API key to version control).
...
└── tick.riv                   # Success/Completion animation, loading questions (Rive file can contain multiple animations , it's up to the developer to choose which animation to use)
